...
1. 트랜잭션1 id = 1 레코드 변경 후 커밋 -> 현재 레코드에 트랜잭션 1 작업 반영
2. 트랜잭션2 id = 1 레코드 변경(커밋전) -> 현재 레코드에 트랜잭션 2 작업 반영 -> 트랜잭션 2 작업 공간에만 반영된것이다. (트랜잭션1 작업은 언두로그)
3. 트랜잭션3 id = 1 조회 (커밋 전) -> 트랜잭션 2가 커밋 전이므로 txId3은 현재 레코드의 값은 txId1이 변경한 값을 보는 것임
4. 트랜잭션4 id = 1 레코드 변경 -> 트랜잭션 2가 commit 전까지 변경할 수 없음
5. 트랜잭션2 id = 1 커밋 (해당 레코드가 txId=2로 변경됨)
6. 트랜잭션3 id = 1 조회 -> 트랜잭션 3을 시작한 순간의 스냅샷을 기반으로 데이터를 읽으므로 트랜잭션 1 데이터를 보게된다.(트랜잭션 2는 트랜잭션 3 시작 전에 커밋하지 않았으므로 트랜잭션 3은 트랜잭션2가 변경한 데이터를 스냅샷에 반영하지 않음)
7. 트랜잭션 3 커밋
8. 트랜잭션 3 다시 id = 1조회 -> 트랜잭션 2가 반영한 데이터 읽음
   ![](./스크린샷%202025-07-10%20오후%209.51.59.png)
   트랜잭션 아이디가 본인 보다 큰게 레코드에 있다면 언두로그에서 본인보다 작은 아이디 값의 가장 최신 데이터를 읽는다. 즉 본인 아니디보다 작은 가장 최댓값을 읽는다

트랜잭션마다 자신의 읽기 타임스탬프(Snapshot) 가 있어서,

자신보다 먼저 COMMIT된 데이터만 읽을 수 있음. 자신의 트랜잭션 시작 전에 커밋된것만 읽을 수 있다는 것
